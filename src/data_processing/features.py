...
def extract_features_from_window(window):
    ...
    for frame in window:
        ...
        if prev_frame is not None:
            ...
            velocity = np.array([v_lwrist, v_rwrist, v_lankle, v_rankle])
        ...
        # --- 4. Symmetry: L–R wrists and ankles ---
    ...
    acceleration = np.array(acceleration) if acceleration else np.zeros_like(velocities)
    symmetry = np.array(symmetry)
    spreads = np.array(spreads)
    motion_energy = np.array(motion_energy)
    joint_stddevs = np.array(joint_stddevs)

    # Aggregate all: mean + std for each feature group
...
def main():
    ...
    X_features = [extract_features_from_window(window) for window in tqdm(X_raw)]
